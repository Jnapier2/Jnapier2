...
| Image Downloader | 2026.07.19.1 | 2026.07.19.1 | Current; verified five-second per-image budget merged into stronger public source |
...
- **Image Downloader 2026.07.19.1:** the verified five-second per-image request budget, three-second connection limit, separate 60-second page-discovery budget, visible-output default, and focused regression tests are merged into the stronger public destination, content-validation, duplicate-control, resume, browser-guardrail, and redaction boundaries. Clean pinned read-only CI passed before merge.

There are no active public source reconciliations. The machine-readable authority is [`.github/release-reconciliation.json`](.github/release-reconciliation.json), and the portfolio health workflow validates its completeness and exact version markers.
